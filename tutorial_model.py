...
data_set = 'clean_v2_extended_no_outliers.csv'
look_back =  730
train_split = 0.7

model_save_file = 'best_model_{}_{}.h5'.format(look_back, data_set[:-4]) 
patience = 500
...
# Haal 12-06-18 uit de data, die ene gekke outlier
data = data.drop(index='12-06-18', axis=0)
index = data.loc[:, 'Date']
...
# raise SystemExit(0)
# --------------------------------------------------------- NA HET ETEN, OUTPUT UIT LSTM -> SIZE -> FEATURES EN DAN EEN PAAR LAGEN DENSE OOK LEN FEFATURES
# Create model and train
samples, time_steps, features = X_train.shape
model = Sequential()
model.add(CuDNNLSTM(128, input_shape=(time_steps, features), return_sequences=True))
...
model.add(Dense(64))
model.add(Dense(32))
# model.add(CuDNNLSTM(int(features/2)))
# model.add(Dense(64))
# model.add(Dense(64))
...
history = model.fit(X_train, Y_train, epochs=no_epochs, validation_split=0.3, callbacks=[es, mc]) #validation_data=(X_test, Y_test))
